...
export function createFlyingEye(
  ...
  const iris = new THREE.Mesh(new THREE.SphereGeometry(0.36, 20, 16), _ph(0xaa66ee, 0x7733cc, 0.7));
...

  {
    ...
    for (const side of [-1, 1] as const) {
      const c = new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.32, 5), _ph(0x2e68cc));
  ...
  return {
    update(dt, playerX, playerZ) {
      time += dt;
      group.position.y = EYE_BASE_Y + Math.sin(time * 1.3) * 0.6;
      group.position.x = EYE_X + Math.sin(time * 0.35) * 1.6;
      iris.rotation.x = Math.sin(time * 0.7) * 0.15;
      iris.rotation.y = Math.sin(time * 0.5) * 0.12;
      pupil.rotation.y = Math.sin(time * 0.55) * 0.1;
...
function clearOfBoxes(x: number, z: number, boxes: BuildingBox[]): boolean {
  const PAD = 2;
  return !boxes.some(
    b => x > b.minX - PAD && x < b.maxX + PAD && z > b.minZ - PAD && z < b.maxZ + PAD,
  );
}
...
function buildCrystalTroll(
  ...
  // ── Chest ───────────────────────────────────────────────────────────────────
  place(root, cap(0.24, 0.14), skin, 0, 1.4, 0);
  ...
  for (const sx of [-1, 1] as const) {
    ...
    const uLen = 0.44;
    place(root, cap(0.09, 0.26), skin, sx * 0.38 + udx * 0.22, 1.66 + udy * 0.22, 0, uRz);
    ...
    const fLen = 0.4;
    place(root, cap(0.085, 0.23), skin, ex + fdx * 0.2, ey + fdy * 0.2, 0, fRz);
  ...
  // ── Neck ────────────────────────────────────────────────────────────────────
  place(root, cap(0.1, 0.06), skin, 0, 1.82, 0);
  ...
  for (const sx of [-1, 1] as const) {
    ...
    ear.position.set(sx * 0.3, 0.08, 0);
    headGroup.add(ear);
  }
...
function buildWingedMonster(
  ...
  for (const sx of [-0.1, 0.1]) {
    ...
    place(root, cap(0.1, 0.2), body, sx * 0.55, 0.74, 0);
  }
  place(root, sph(0.15), body, 0, 0.9, 0);

  // ── Chest (narrow) ──────────────────────────────────────────────────────────
  place(root, cap(0.17, 0.14), body, 0, 1.2, 0);
  place(root, sph(0.1), body, -0.3, 1.46, 0);
  place(root, sph(0.1), body, 0.3, 1.46, 0);
  ...
  for (const sx of [-1, 1] as const) {
    ...
    e.position.set(sx * 0.1, 0.04, 0.18);
    headGroup.add(e);
  }
  ...
  function buildWing(parent: THREE.Group, side: -1 | 1): void {
    const armSpar = new THREE.Mesh(cap(0.04, 0.8), spar);
    ...
    const mem = new THREE.Mesh(new THREE.PlaneGeometry(1.3, 0.92), wMem);
...
export function createMonsters(
  ...
  return {
    ...
    update(dt: number): void {
      ...
      troll.headGroup.rotation.y = Math.sin(time * 0.55) * 0.3;
      troll.rootGroup.rotation.z = Math.sin(time * 0.4) * 0.035;

      winged1.headGroup.rotation.y = Math.sin(time * 0.45) * 0.25;
      const w1Angle = Math.sin(time * 0.9) * 0.4;
      ...
      const w2Angle = Math.sin(time * 0.85 + 0.5) * 0.4;
